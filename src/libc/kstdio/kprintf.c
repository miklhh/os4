...
/* Help function for converting a 32-bit unsigned integer to a hex-string */
static char* hexconverter (uint32_t num)
{
	memcpy(
        hexstring, 
        (char[11]) { '0', 'x', '0', '0', '0', '0', '0', '0', '0', '0', '\0'}, 
        11);
...
int kprintf(const char* restrict format, ...) 
{
	...
	while (*format != '\0') 
	{
		...
		/* Handle '%x', an unsinged integer being passed in the
		...
		else if (*format == 'u')
		{
			...
			for (size_t i = 0; i < index; i++)
			{
                ...
                char current_number[1] = { stack[index - (i+1)] + 48 };
